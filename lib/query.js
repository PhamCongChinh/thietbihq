...
export const getProducts = async () => {
    try {
        const results = await sql_query(
            `SELECT p.id, p.name, p.slug FROM products as p ORDER BY id DESC`
        )
...
export const getProductsByCategory = async (slug) => {
    ...
    try {
        const results = await sql_query(
            `SELECT p.id, p.name, p.slug FROM products as p WHERE p.category_id = ${id}`
        )
        const products = Object.values(JSON.parse(JSON.stringify(results)))
        return products
    } catch (error) {
...
export const getRelatedProducts = async (id) => {
    try {
        const results = await sql_query(
            `SELECT * FROM products as p WHERE p.category_id = ${id}`
        )
        const products = Object.values(JSON.parse(JSON.stringify(results)))
        return products
    } catch (error) {
