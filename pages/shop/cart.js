...
const Cart = () => {
    ...
    const cartsCount = useSelector(state => state.cart.cartTotalQuantity)
                ...
                {carts.map(item => (
                    <div key={item.id} className="grid border-b py-2 md:grid-cols-10">
                        <div className="pt-1 md:col-span-1">
                            <Image src={`/images/pr/${item.slug}.jpg`} alt={item.name} height={70} width={70} />
                        </div>
                        <div className="py-1 md:col-span-6 flex items-center">
